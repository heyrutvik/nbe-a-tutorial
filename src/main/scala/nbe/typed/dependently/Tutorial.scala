...
object Tutorial {
  ...
  case class IndNat(target: Expr, motive: Expr, base: Expr, step: Expr) extends Expr // eliminator
  // equality type
  ...
  case class NIndNat(target: Neutral, motive: Norm, base: Norm, step: Norm) extends Neutral
  ...
  // ctx->env
  def ctxToRho(ctx: Ctx): Rho = ctx.map { case (name, context) =>
    context match {
  ...
  def isAlphaEquiv(e1: Expr, e2: Expr): Boolean = {
    ...
    // α-equiv-aux
    def isAlphaEquivAux(e1: Expr, e2: Expr, s1: Sym, s2: Sym): Boolean = (e1, e2) match {
      case (Var(x), Var(y)) =>
      ...
      case (Pi(x, e1, b1), Pi(y, e2, b2)) =>
        val fx = gensym
        isAlphaEquivAux(e1, e2, s1, s2) && isAlphaEquivAux(b1, b2, extend(s1, x, fx), extend(s2, y, fx))
      ...
      case (Cons(fst1, snd1), Cons(fst2, snd2)) =>
        isAlphaEquivAux(fst1, fst2, s1, s2) && isAlphaEquivAux(snd1, snd2, s1, s2)
  ...
  def eval(p: Rho, e: Expr): Value = e match {
    ...
    case IndNat(target, motive, base, step) =>
      applyIndNat(eval(p, target), eval(p, motive), eval(p, base), eval(p, step))
    case Eq(t, w1, w2) =>
  ...
  // do-replace
  def applyReplace(target: Value, motive: Value, base: Value): Value = target match {
    case SAME => base
  ...
  def indNatStepType(motive: Value): Value = {
    PI(NAT, HOCLOS('n, n => PI(apply(motive, n), HOCLOS('ih, ih => apply(motive, ADD1(n))))))
  }

  // do-ind-Nat
  def applyIndNat(target: Value, motive: Value, base: Value, step: Value): Value = target match {
    case ZERO => base
  ...
  def readBackNorm(ctx: Ctx, norm: Norm): Expr = norm match {
    ...
    case THE(PI(pt, rt), f) =>
      ...
      Lam(fx, readBackNorm(extendCtx(ctx, fx, pt), THE(evalClosure(rt, nfx), apply(f, nfx))))
    case THE(SI(t1, t2), p) =>
    ...
    case THE(UNI, SI(t1, t2)) =>
      ...
      val fx = freshen(ctx.map(_._1), x)
      Si(fx, readBackNorm(ctx, THE(UNI, t1)), readBackNorm(extendCtx(ctx, fx, t1), THE(UNI, evalClosure(t2, NEU(t1, NVar(fx))))))
    case THE(UNI, PI(pt, rt)) =>
      val x = closureName(rt)
      val fx = freshen(ctx.map(_._1), x)
      Pi(fx, readBackNorm(ctx, THE(UNI, pt)), readBackNorm(extendCtx(ctx, fx, pt), THE(UNI, evalClosure(rt, NEU(pt, NVar(fx))))))
    case THE(UNI, UNI) => U
  ...
  def readBackNeutral(ctx: Ctx, neu: Neutral): Expr = neu match {
    ...
    case NIndAbsurd(target, motive) =>
      IndAbsurd(The(Absurd, readBackNeutral(ctx, target)), readBackNorm(ctx, motive))
  }
  ...
  def synthesis(ctx: Ctx, e: Expr): Go[The] = e match {
    ...
    case App(e1, e2) =>
      synthesis(ctx, e1).flatMap { case The(e1t, e2o) =>
        eval(ctxToRho(ctx), e1t) match {
          case PI(pt, rt) =>
            checking(ctx, e2, pt).flatMap { ro =>
              Right(The(readBackNorm(ctx, THE(UNI, evalClosure(rt, eval(ctxToRho(ctx), ro)))), App(e2o, ro)))
            }
          case v =>
    ...
    case Car(p) =>
      synthesis(ctx, p).flatMap { case The(pt, po) =>
      ...
      synthesis(ctx, p).flatMap { case The(pt, po) =>
        eval(ctxToRho(ctx), pt) match {
          case SI(t1, t2) =>
            ...
            Right(The(readBackNorm(ctx, THE(UNI, evalClosure(t2, tc))), Cdr(po)))
          case v =>
            val t = readBackNorm(ctx, THE(UNI, v))
            Left(s"Expected Σ, got $t")
        }
    ...
    case Replace(target, motive, base) =>
      synthesis(ctx, target).flatMap { case The(tt, to) =>
        eval(ctxToRho(ctx), tt) match {
          case EQ(t, w1, w2) =>
            ...
            } yield (The(readBackNorm(ctx, THE(UNI, apply(mv, w2))), Replace(to, mo, bo)))
          case v =>
            val t = readBackNorm(ctx, THE(UNI, v))
    ...
    case IndNat(target, motive, base, step) =>
      ...
      } yield (The(readBackNorm(ctx, THE(UNI, apply(mv, eval(ctxToRho(ctx), to)))), IndNat(to, mo, bo, so)))
  }
  ...
  def checking(ctx: Ctx, e: Expr, t: Value): Go[Expr] = e match {
    case Lam(x, b) =>
      t match {
        case PI(pt, rt) =>
          ...
          for (bo <- checking(extendCtx(ctx, x, pt), b, evalClosure(rt, nx))) yield {
            Lam(x, bo)
          }
        case v =>
    ...
    case _ =>
      synthesis(ctx, e).flatMap { case The(to, eo) =>
  ...
  def interact(ctx: Ctx, input: Expr): Go[Ctx] = input match {
    case Let(x, e) =>
      lookup(ctx, x) match {
        ...
        case None =>
          synthesis(ctx, e).map { case The(t, e1) =>
            val p = ctxToRho(ctx)
...
object Implicits {
  ...
  implicit class ExprOps(e: Expr) {
    def notation: String = e match {
      ...
      case Eq(t, w1, w2) => s"(= (${t.notation}) (${w1.notation}) (${w2.notation}))"
      case Same => "same"
      case Replace(ta, mo, ba) => s"(replace (${ta.notation}) (${mo.notation}) (${ba.notation}))"
      case Trivial => "Trivial"
      case Sole => "sole"
      case Absurd => "Absurd"
      case IndAbsurd(at, t1) => s"(ind-Absurd (${at.notation}) (${t1.notation}))"
      case Atom => "Atom"
      case Quote(s) => s"'$s"
      case U => "U"
