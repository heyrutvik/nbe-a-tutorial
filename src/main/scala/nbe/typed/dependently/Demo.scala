...
object Demo extends App {
  ...
  val the_id = The(Pi('x, U, Pi('y, Var('x), Var('x))), Lam('a, Lam('b, Var('b))))
  ...
  val rep1 = Replace(The(Eq(Nat, Add1(Zero), Add1(Zero)), Same), Lam('x, Atom), Var('a))
  ...
  val let_NatEqConsequencesRefl = Let('natEqConsequencesRefl, the_NatEqConsequencesRefl)
  ...
  val the_thereAreConsequences = The(
    Pi('j, Nat, Pi('k, Nat, Pi('jk, Eq(Nat, Var('j), Var('k)), App(App(Var('natEqConsequences), Var('j)), Var('k))))),
    Lam('j,
      Lam('k,
        Lam('jk,
          Replace(
  ...
  val let_thereAreConsequences = Let('thereAreConsequences, the_thereAreConsequences)
  ...
  val tac4 = App(App(App(Var('thereAreConsequences), Add1(Zero)), Add1(Zero)), Same)
  val tac5 = App(App(Var('thereAreConsequences), Zero), Add1(Zero))
  val tac6 = App(App(Var('thereAreConsequences), Add1(Zero)), Zero)

  run(
    List(
      let_id, the_id, app_typ, app_val, the_u_cons, let_cons,
      the_cons, the_car, the_cdr, the_atom, let_atom, the_p1,
      the_same,
      rep1,
      indn1,
      the_NatEqConsequences, let_NatEqConsequences,
      the_NatEqConsequencesRefl, let_NatEqConsequencesRefl,
      the_thereAreConsequences, let_thereAreConsequences,
      necr1, necr2, tac1, tac2, tac3, tac4, tac5, tac6
    )
  )
